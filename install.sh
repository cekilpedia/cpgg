...
curl --silent \
  ...
  https://api.github.com/repos/cekilpedia/autoscript/releases/latest |
  grep '"tag_name":' |
  sed -E 's/.*"([^"]+)".*/\1/'
}
...
SUPPORT_LINK="https://instagram.com/cekilygy"
WIKI_LINK="https://github.com/cekilpedia/autoscript/wiki"
GITHUB_URL="https://raw.githubusercontent.com/cekilpedia/autoscript/$SCRIPT_RELEASE"
...
centos_php() {
curl -so /etc/php-fpm.d/www-controlpanel.conf "$GITHUB_URL"/cekilxd/www-controlpanel.conf
...
curl -so /var/www/controlpanel/.env.example "$GITHUB_URL"/cekilxd/.env.example
...
case "$OS" in
  debian | ubuntu)
    ...
    curl -so /etc/nginx/sites-available/controlpanel.conf "$GITHUB_URL"/cekilxd/$WEB_FILE
  ...
  centos)
    ...
    curl -so /etc/nginx/conf.d/controlpanel.conf "$GITHUB_URL"/cekilxd/$WEB_FILE
...
curl -so /etc/systemd/system/controlpanel.service "$GITHUB_URL"/cekilxd/controlpanel.service
